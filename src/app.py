...
class data:
    ...
    def Genre(self,genre,lang):
        with engine.connect() as connection:
            query= text(f"SELECT id,title,poster_path,recommendations FROM main_table WHERE original_language='{lang}' AND genres LIKE '%{genre}%'AND release_date>=DATE '2015-01-01'  AND poster_path IS NOT NULL ORDER BY release_date DESC,vote_average DESC,result ASC;")
